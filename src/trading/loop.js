...
setInterval(async()=>{
    ...
    // ── Simple engine: órdenes reales se disparan vía setOrderCallbacks ──
    // Las llamadas a placeLiveBuy/placeLiveSell viven en los callbacks
    // cableados en server.js (initBot). Aquí solo evaluamos + persistimos.
    // Eliminado el bloque polling-log BUY-only (bug: SELLs nunca se ejecutaban).
    if(S.simpleBot && !S.tgControls?.isPaused() && !S.bot._pausedByTelegram) {
      S.simpleBot.evaluate();
      // Save simple state every 6 ticks (~60s)
      if(ticks%6===0) {
