...
class SimpleBotEngine {
  ...
  // ── T1: Helpers de test — _forceTestBuy/_forceTestSell ─────────────────
  // Sólo usados por el endpoint temporal /api/test/force-signal en PAPER-LIVE.
  // Reproducen el camino atómico de _onCandleClose pero forzando el BUY sin
  // evaluar signal/ATR/correlation. NO se exportan públicamente y el endpoint
  // que los invoca se elimina del código después de validar el pipeline.
  // NUNCA invocar estos métodos desde código que no sea el endpoint temporal.
  _forceTestBuy(cfg, ignorarFiltros = true) {
    if (!cfg) return { ok: false, error: "cfg required" };
    if (this.portfolio[cfg.id]) return { ok: false, error: "position already open" };
    if (Date.now() < (this._capitalSyncPausedUntil || 0))
      return { ok: false, error: "capital sync paused" };
    const price = this.prices[cfg.pair];
    if (!price || price <= 0) return { ok: false, error: `no price for ${cfg.pair}` };

    const tv = this.totalValue();
    const capDeclarado = this._capitalDeclarado || INITIAL_CAPITAL;
    const capEfectivo  = this._capitalEfectivo  || capDeclarado;
    const capRef       = Math.min(capDeclarado, capEfectivo);
    const sizingBase   = Math.min(tv, capRef);
    // Usar kelly rolling (igual que _onCandleClose) en vez de cfg.kelly estático
    const kellyRolling = calcKelly(this._stratTrades[cfg.id] || []);
    const kellyFrac    = Math.max(0.05, Math.min(0.5, kellyRolling.kelly || 0.1));
    let invest = sizingBase * kellyFrac * 0.5;
    if (invest > sizingBase * 0.30) invest = sizingBase * 0.30;
    const availCash = cfg.capa === 1 ? this.capa1Cash : this.capa2Cash;
    if (invest > availCash) invest = availCash;

    const committed = Object.values(this.portfolio).reduce((s,p)=>s+(p.invest||0), 0);
    const capLimit = capRef * 1.005;
    if (committed + invest > capLimit) {
      const headroom = capLimit - committed;
      if (headroom < 10) return { ok: false, error: `cap exceeded (headroom=${headroom.toFixed(2)})` };
      invest = headroom;
    }
    if (invest < 10) return { ok: false, error: `invest=${invest.toFixed(2)} < $10 min` };
    ...
    this.log.push({ type:"BUY", symbol: cfg.pair, strategy: cfg.id, price, invest, ts: Date.now(), test: true });
    console.log(`[SIMPLE][TEST-BUY] ${cfg.pair} @ $${price.toFixed(4)} $${invest.toFixed(2)} [Capa${cfg.capa}] ${cfg.id} (ignorarFiltros=${ignorarFiltros})`);

    if (typeof this._onBuy === "function") {
      try { this._onBuy(cfg.pair, invest, { strategyId: cfg.id, capa: cfg.capa, expectedPrice: price }); }
      catch(e) { console.error(`[SIMPLE][TEST-BUY][onBuy]`, e.message); }
    }
    return {
      ok: true, strategyId: cfg.id, pair: cfg.pair, price, invest, qty,
      capa1CashAfter: +this.capa1Cash.toFixed(4),
      capa2CashAfter: +this.capa2Cash.toFixed(4),
      portfolio: Object.keys(this.portfolio),
    };
  }

  _forceTestSell(cfg) {
    if (!cfg) return { ok: false, error: "cfg required" };
    const pos = this.portfolio[cfg.id];
    if (!pos) return { ok: false, error: "no position open" };
    const price = this.prices[pos.pair] || pos.entryPrice;
    const pnlPct = (price - pos.entryPrice) / pos.entryPrice * 100;
    const gross = pos.qty * price;
    const expectedNet = gross * (1 - FEE);
    if (pos.capa === 1) this.capa1Cash += expectedNet;
    else                this.capa2Cash += expectedNet;
    if (!this._stratTrades[cfg.id]) this._stratTrades[cfg.id] = [];
    this._stratTrades[cfg.id].push({ pnl: pnlPct, ts: Date.now() });
    this.log.push({ type:"SELL", symbol: pos.pair, strategy: cfg.id, pnl: pnlPct, reason: "TEST", ts: Date.now(), test: true });
    console.log(`[SIMPLE][TEST-SELL] ${pos.pair} P&L:${pnlPct.toFixed(2)}% (forced)`);
    const sellCtx = {
      strategyId: cfg.id, capa: pos.capa, pair: pos.pair,
      qty: pos.qty, entryPrice: pos.entryPrice,
      expectedGross: gross, expectedNet, reason: "TEST",
    };
    delete this.portfolio[cfg.id];
    if (typeof this._onSell === "function") {
      try { this._onSell(pos.pair, pos.qty, sellCtx); }
      catch(e) { console.error(`[SIMPLE][TEST-SELL][onSell]`, e.message); }
    }
    return {
      ok: true, strategyId: cfg.id, pair: pos.pair, price, pnlPct,
      expectedNet: +expectedNet.toFixed(4),
      capa1CashAfter: +this.capa1Cash.toFixed(4),
      capa2CashAfter: +this.capa2Cash.toFixed(4),
      portfolio: Object.keys(this.portfolio),
    };
